...
class TestICMP(tests.BaseTest):
    ...
    def evaluate(self, packets):
        ...
        typecodes = [[1,None],[2,None],[3,0],[4,1],[4,2],[128,None],[129,None]]
        ...
        for number in [1,2]:
            if [number, None] in typecodes:
                results.append("Success! The firewall FORWARDED an ICMPv6 packet with type %d (%s). This is technically correct, but as a message with type %d is usually in response to a request (and our server didn't even send that request) you can't be sure this behaviour is fine." % (number, self.ICMPTypes[number], number))
            else:
        ...
        if [3,0] in typecodes:
            results.append("Success! The firewall FORWARDED an ICMPv6 packet with type 3 code 0 (%s). This is technically correct, but as a message with type 3, code 0 is usually in response to a request (and our server didn't even send that request) you can't be sure this behaviour is fine." % self.ICMPTypes[3])
        else:
        ...
        if [4,1] in typecodes:
            results.append("Success! The firewall FORWARDED an ICMPv6 packet with type 4 code 1 (%s). This is technically correct, but as a message with type 4, code 1 is usually in response to a request (and our server didn't even send that request) you can't be sure this behaviour is fine." % self.ICMPTypes[3])
        else:
        ...
        if [4,2] in typecodes:
            results.append("Success! The firewall FORWARDED an ICMPv6 packet with type 4 code 2 (%s). This is technically correct, but as a message with type 4, code 2 is usually in response to a request (and our server didn't even send that request) you can't be sure this behaviour is fine." % self.ICMPTypes[3])
        else:
    ...
    def send_mandatory(self):
        ...
        payload="ipv6-qab" * 12 # "1024 Bytes of Payload
