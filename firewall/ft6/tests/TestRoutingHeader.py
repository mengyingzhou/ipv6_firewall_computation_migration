...
class TestRoutingHeader(tests.BaseTest):
    ...
    def execute(self):
        ...
        payload="ipv6-qab"*12 # 1024 Bytes Payload
    ...
    def evaluate(self, packets):
        ...
        if '4' in steps:
            results.append("Success! The firewall FORWARDED a 'routing header type 2' packet with 'segments left' set to one. However, you should only allow Routing Header Type 2 if you need MobileIP Support.");
        else:
            results.append("Failure! The firewall DROPPED a 'routing header type 2' packet with 'segments left' set to one. Dropping this valid header is in some way better than forwarding the invalid header!")
        
        
            
        # RH 200 with segments left = 0 may make it to the other side
        if '5' in steps:
            results.append("Success! The firewall FORWARDED an unallocated routing header type 200 (with 'segments left' set to zero). You should filter those unless you really need it")
        else:
